 ...
 */
public class McLeodMethod implements PitchDetector {
     ...
     */
    public McLeodMethod(final float audioSampleRate) {
     ...
     */
    public McLeodMethod(final float audioSampleRate, final int audioBufferSize) {
     ...
     *            paper 0.93 is used.
     */
    public McLeodMethod(final float audioSampleRate, final int audioBufferSize, final double cutoffMPM) {
    ...
    public PitchDetectionResult getPitch(final float[] audioBuffer) {
        ...
        for (final Integer tau : maxPositions) {
            ...
            if (nsdf[tau] > SMALL_CUTOFF) {
                // calculates turningPoint X and Y
